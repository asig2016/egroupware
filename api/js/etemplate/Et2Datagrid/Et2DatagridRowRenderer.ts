...
export class Et2DatagridRowRenderer
{
		...
		try
		{
			...
			for(const element of toUpgrade)
			{
				try
				{
					...
					this.applyRowSelectOptions(element, stored);
				}
				catch(e)
				{
	 ...
	 */
	private applyRowSelectOptions(element : any, stored : Record<string, any> | undefined)
	{
		...
		const manager = this.host.getArrayMgr?.("sel_options");
