 ...
 * and it does not terminate. Both re-entrancy guards involved (`_syncingFromNextmatch` here and
 * `et2_nextmatch.update_in_progress` there) are cleared synchronously, but a widget dispatches
 * its "change" only after its own updateComplete - a later microtask than the sync window - so
 * the echo always arrives with both guards already down. The result is a hung renderer with no
