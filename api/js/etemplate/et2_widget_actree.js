        ...
        if (!this.options.readonly) {
            buttons.push({
                ...
                click   : function() {
					...
					that.set_value({ id: idNode, app: that.options.only_app});
                    that.aclink.set_value({ id: idNode, app: that.options.only_app, title: `#${idNode}` });
					console.log('aclink set value'+idNode.toString()+' ...'+that.options.only_app.toString()+ typeof that.aclink + ' value' + that.get_value().toString());
