...
class Lib
{
    ...
    static function login( $params ) {
        ...
        try {
            ...
            throw new \Exception('Authentication Failed - Debug Message:'.$e->getMessage());
        }
    }
    ...
    static function getMail( $profile, $emailID ) {
		...
		$items = get_object_vars( $response->ResponseMessages->GetItemResponseMessage->Items );
		$key = array_keys( $items );
		$item = array_pop( $items );
		// Probably Meeting Request.
		// Not supporting fully. Prepend important info to body
		if ( $key[0] != 'Message' ) {
			$prepend = '';
			$fields = array('Duration','Location','Start','End');
			foreach( $fields as $field ) {
				if ( isset( $item->$field ) ) {
					...
					case 'Duration':
						...
						$value = $interval->format('%h hours, %i minutes');
						break;
					case 'Start':
					case 'End':
						$time = strtotime( $value );
						$format = $GLOBALS['egw_info']['user']['preferences']['common']['dateformat'].' H:i';
						$value = date( $format, $time );
						break;
					}
					$prepend .= "<b>$field:</b> $value <br \>";
				}
			}
			$prepend .= '<br \>';

			$item->Body->_ = $prepend . $item->Body->_;
		}
    ...
    static function getMailRaw( $profile, $emailID ) {
        ...
        $responseMsg = $response->ResponseMessages->GetItemResponseMessage;
        if ($responseMsg->ResponseCode == 'NoError' &&
            $responseMsg->ResponseClass == 'Success') {
			$items = get_object_vars( $response->ResponseMessages->GetItemResponseMessage->Items );
			$mail = array_pop( $items );
            return base64_decode( $mail->MimeContent->_ );
        }
        else
            return '';
    }
    ...
    static function getMails( $profile, $folderID, $start, $num_rows, $sort, $filter, $id_only = false ) {
        ...
        //Format
		$emails = array();
		$items = get_object_vars( $response->ResponseMessages->FindItemResponseMessage->RootFolder->Items );
		foreach( $items as $item ) {
			if ( $item && is_array($item) === FALSE ) {
				$item = array( $item );
			}
			$emails = array_merge( $emails, $item );
		}
		$field = explode(':', $sort['order'] )[1];
		$order = ( $order == 'Descending' ? 1 : -1 );
		usort( $emails, function($a, $b) use ($field,$order) {
			$x = $a->$field;
			$y = $b->$field;
			if ($x == $y) {
				return 0;
			}
			return ($x < $y) ? -1*($order) : $order ;
		});
